...
public class halfDayLeaveForm extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        LEAVE_ID = sharedPreferences.getString("empId", null) + getTime("yyMMddHHmmssMS");
        findViewById(R.id.backBtnHDL).setOnClickListener(view -> finish());
        ...
        firebaseFirestore.collection("employees")
                ...
                .get().addOnCompleteListener(task -> {
                    if(Objects.requireNonNull(task.getResult().getDouble("Casual Leave"))>0)leaveAvailable.add("Casual Leave");
        ...
        leaveDateTV=findViewById(R.id.leaveDateTV_HDL);
        ...
        datePickerDialog.getDatePicker().setMaxDate(Objects.requireNonNull(getTimeStamp(calendar.get(Calendar.YEAR)+"-12-31T00:00:00Z")).getTime());
        leaveDateTV.setOnClickListener(v -> datePickerDialog.show());
        leaveReason=findViewById(R.id.leaveReason_HDL);
        ...
        imageView = findViewById(R.id.applicationDocHDL);
        storageReference = FirebaseStorage.getInstance().getReference();
        findViewById(R.id.submitBtnHDL).setOnClickListener(v -> {
            uploadImage();
            applyLeave();
        });
    }
    ...
    private void applyLeave() {
        RadioGroup timePeriod=findViewById(R.id.timePeriodRadioGroup);
        if (isConnectionAvailable(getApplicationContext())) startActivity(new Intent(this, noInternetActivity.class));
        ...
        } else if (timePeriod.getCheckedRadioButtonId()==-1) Toast.makeText(this, "Select leave time period", Toast.LENGTH_SHORT).show();
            ...
            leaveApplication.put("timePeriod",radioButton.getText());
            ...
            leaveApplication.put("leaveCategory","Half Day Leave");
            leaveApplication.put("numberOfLeave",0.5);
            ...
            firebaseFirestore.collection("leaveApplied")
                    ...
                    .addOnCompleteListener(task -> {
                        ...
                        if(!(leaveType.equals("Duty Leave")||leaveType.equals("Research Leave"))) firebaseFirestore.collection("employees")
                                ...
                                .collection("leaveBalance")
                                .document("remainLeaveBalanceChart")
                                .update(leaveType,FieldValue.increment(-0.5));
                        firebaseFirestore.collection("employees")
                                .document(sharedPreferences.getString("empId", null))
                                .collection("leaveBalance")
                                ...
                                .addOnCompleteListener(task1 -> {
                                    ...
                                    miscellaneousMethods.leaveApplied(this,this,"Leave Applied","Application Id:","HDL"+LEAVE_ID,"Leave Date", leaveDateTV.getText().toString(),"Leave Type:",leaveSpinner.getSelectedItem().toString());
                                });
                    })
